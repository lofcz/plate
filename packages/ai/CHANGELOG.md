...
- c8487e0: Make AI change tracking calmer. Changed blocks get one soft tint, with no side bar, word marks or removal markers. The tint fades out for good after the user rests the pointer on the block (`acknowledgeDelay`, default 600 ms), edits inside it, or starts a new AI session.
...
- ea98a5c: Rework direct AI edits around a structural diff and a minimal live-tree commit, so unaffected nodes keep their identity and large documents no longer lag while edits apply.
...
- 1ea9a7e: Reveal document rewrites with a soft directional sweep and a readable initial hold within the total animation budget. Keep section insertions on the local animation path and support application-owned atomic commits and collapsed-section reveal targeting.
...
- c08f1da: Block-level diff with word-level hints (new-above-old)
...
- [#4800](https://github.com/udecode/plate/pull/4800) by [@felixfeng33](https://github.com/felixfeng33) – Upgraded AI SDK from v5 to v6:
