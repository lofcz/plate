...
- 27cc06d: Fix critical source-map bug: blocks whose body was filtered out by
...
- a275472: Fix `parseAttributes` / `propsToAttributes` mishandling of MDX boolean attributes and stop the self-amplifying junk-attribute leak across serialize → deserialize round-trips.
...
- 246f8ea: Hardens the markdown source-map so `resolveSelectionByPath` can never silently report invalid line ranges. Each fix is locked in by a slow-suite regression test.
...
- 6497ca0: Fix source map generation for tables inside MDX containers and add `resolveSelectionByPath`
...
- [#4620](https://github.com/udecode/plate/pull/4620) by [@baptisteArno](https://github.com/baptisteArno) – Fix remarkStringifyOptions not taken into account
