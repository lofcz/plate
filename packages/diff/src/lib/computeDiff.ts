...
export const computeDiff = (
  ...
  // The char mapping is a strict bijection: distinct node values (modulo
  // `ignoreProps`) get distinct chars. Prop-only updates (e.g. a renamed
  // activity) therefore surface as DMP delete+insert regions, and are
  // reconciled into granular update marks downstream by
  // `pairBlocksWithWordHints` via the strategy's `updateProps`.
