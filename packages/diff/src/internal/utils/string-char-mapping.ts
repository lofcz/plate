...
export type StringCharMappingOptions = {
   ...
   * `ComputeDiffOptions`.
   *
   * NOTE: the char mapping is strictly a bijection between distinct node
   * values and chars. An earlier revision experimented with "structural
   * key" aliasing (two byte-unequal nodes sharing one char so DMP would
   * keep them paired), but a char can only resolve back to ONE node — the
   * aliasing made `stringToNodes` return the first-registered node for
   * every occurrence, duplicating content and cross-pairing unrelated
   * blocks that happened to share a key. Prop-only updates (renames) are
   * instead reconciled downstream in `pairBlocksWithWordHints` /
   * `handleReplacePair` via the strategy's `updateProps`, which works on
   * the real positional nodes of a delete+insert region.
   */
  ignoreProps?: string[];
};
